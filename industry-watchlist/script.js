const industryData = [
  {
    "id": "881001",
    "code": "881001",
    "name": "煤炭",
    "children": [
      {
        "id": "881002",
        "code": "881002",
        "name": "煤炭开采",
        "children": [
          {
            "id": "881003",
            "code": "881003",
            "name": "动力煤"
          },
          {
            "id": "881004",
            "code": "881004",
            "name": "炼焦煤"
          }
        ]
      },
      {
        "id": "881005",
        "code": "881005",
        "name": "焦炭加工",
        "children": []
      }
    ]
  },
  {
    "id": "881006",
    "code": "881006",
    "name": "石油",
    "children": [
      {
        "id": "881007",
        "code": "881007",
        "name": "油气开采",
        "children": []
      },
      {
        "id": "881011",
        "code": "881011",
        "name": "石油化工",
        "children": [
          {
            "id": "881012",
            "code": "881012",
            "name": "炼油化工"
          },
          {
            "id": "881013",
            "code": "881013",
            "name": "石油贸易"
          },
          {
            "id": "881014",
            "code": "881014",
            "name": "其他石化"
          }
        ]
      },
      {
        "id": "881008",
        "code": "881008",
        "name": "油服工程",
        "children": [
          {
            "id": "881009",
            "code": "881009",
            "name": "油田服务"
          },
          {
            "id": "881010",
            "code": "881010",
            "name": "油气工程"
          }
        ]
      }
    ]
  },
  {
    "id": "881015",
    "code": "881015",
    "name": "化工",
    "children": [
      {
        "id": "881016",
        "code": "881016",
        "name": "日用化工",
        "children": [
          {
            "id": "881017",
            "code": "881017",
            "name": "洗护用品"
          },
          {
            "id": "881018",
            "code": "881018",
            "name": "化妆品"
          }
        ]
      },
      {
        "id": "881019",
        "code": "881019",
        "name": "化纤",
        "children": [
          {
            "id": "881020",
            "code": "881020",
            "name": "涤纶"
          },
          {
            "id": "881021",
            "code": "881021",
            "name": "氨纶"
          },
          {
            "id": "881022",
            "code": "881022",
            "name": "粘胶纤维"
          },
          {
            "id": "881023",
            "code": "881023",
            "name": "锦纶"
          },
          {
            "id": "881024",
            "code": "881024",
            "name": "碳纤维"
          },
          {
            "id": "881025",
            "code": "881025",
            "name": "其他化学纤维"
          }
        ]
      },
      {
        "id": "881026",
        "code": "881026",
        "name": "化学原料",
        "children": [
          {
            "id": "881027",
            "code": "881027",
            "name": "煤化工"
          },
          {
            "id": "881028",
            "code": "881028",
            "name": "钛白粉"
          },
          {
            "id": "881029",
            "code": "881029",
            "name": "氯碱"
          },
          {
            "id": "881030",
            "code": "881030",
            "name": "纯碱"
          },
          {
            "id": "881031",
            "code": "881031",
            "name": "无机盐"
          },
          {
            "id": "881032",
            "code": "881032",
            "name": "有机化工原料"
          },
          {
            "id": "881033",
            "code": "881033",
            "name": "其他化学原料"
          }
        ]
      },
      {
        "id": "881034",
        "code": "881034",
        "name": "化学制品",
        "children": [
          {
            "id": "881035",
            "code": "881035",
            "name": "聚氨酯"
          },
          {
            "id": "881036",
            "code": "881036",
            "name": "涂料油墨"
          },
          {
            "id": "881037",
            "code": "881037",
            "name": "民爆制品"
          },
          {
            "id": "881038",
            "code": "881038",
            "name": "纺织化学品"
          },
          {
            "id": "881039",
            "code": "881039",
            "name": "氟化工"
          },
          {
            "id": "881040",
            "code": "881040",
            "name": "食品助剂"
          },
          {
            "id": "881041",
            "code": "881041",
            "name": "有机硅"
          },
          {
            "id": "881042",
            "code": "881042",
            "name": "胶粘剂及胶带"
          },
          {
            "id": "881043",
            "code": "881043",
            "name": "其他化学制品"
          }
        ]
      },
      {
        "id": "881044",
        "code": "881044",
        "name": "塑料",
        "children": [
          {
            "id": "881045",
            "code": "881045",
            "name": "合成树脂"
          },
          {
            "id": "881046",
            "code": "881046",
            "name": "塑料薄膜"
          },
          {
            "id": "881047",
            "code": "881047",
            "name": "改性塑料"
          },
          {
            "id": "881048",
            "code": "881048",
            "name": "合成革"
          },
          {
            "id": "881049",
            "code": "881049",
            "name": "塑料助剂"
          },
          {
            "id": "881050",
            "code": "881050",
            "name": "其他塑料制品"
          }
        ]
      },
      {
        "id": "881051",
        "code": "881051",
        "name": "橡胶",
        "children": [
          {
            "id": "881052",
            "code": "881052",
            "name": "橡胶助剂"
          },
          {
            "id": "881053",
            "code": "881053",
            "name": "炭黑"
          },
          {
            "id": "881054",
            "code": "881054",
            "name": "橡胶制品"
          }
        ]
      },
      {
        "id": "881055",
        "code": "881055",
        "name": "农用化工",
        "children": [
          {
            "id": "881056",
            "code": "881056",
            "name": "农药"
          },
          {
            "id": "881057",
            "code": "881057",
            "name": "氮肥"
          },
          {
            "id": "881058",
            "code": "881058",
            "name": "磷肥及磷化工"
          },
          {
            "id": "881059",
            "code": "881059",
            "name": "钾肥"
          },
          {
            "id": "881060",
            "code": "881060",
            "name": "复合肥"
          }
        ]
      },
      {
        "id": "881104",
        "code": "881104",
        "name": "非金属材料",
        "children": []
      }
    ]
  },
  {
    "id": "881061",
    "code": "881061",
    "name": "钢铁",
    "children": [
      {
        "id": "881062",
        "code": "881062",
        "name": "冶钢原料",
        "children": [
          {
            "id": "881063",
            "code": "881063",
            "name": "铁矿石"
          },
          {
            "id": "881064",
            "code": "881064",
            "name": "冶钢辅料"
          }
        ]
      },
      {
        "id": "881065",
        "code": "881065",
        "name": "普钢",
        "children": [
          {
            "id": "881066",
            "code": "881066",
            "name": "长材"
          },
          {
            "id": "881067",
            "code": "881067",
            "name": "板材"
          },
          {
            "id": "881068",
            "code": "881068",
            "name": "其他钢材"
          }
        ]
      },
      {
        "id": "881069",
        "code": "881069",
        "name": "特钢",
        "children": []
      }
    ]
  },
  {
    "id": "881070",
    "code": "881070",
    "name": "有色",
    "children": [
      {
        "id": "881071",
        "code": "881071",
        "name": "工业金属",
        "children": [
          {
            "id": "881072",
            "code": "881072",
            "name": "铜"
          },
          {
            "id": "881073",
            "code": "881073",
            "name": "铝"
          },
          {
            "id": "881074",
            "code": "881074",
            "name": "铅锌"
          }
        ]
      },
      {
        "id": "881075",
        "code": "881075",
        "name": "贵金属",
        "children": [
          {
            "id": "881076",
            "code": "881076",
            "name": "黄金"
          },
          {
            "id": "881077",
            "code": "881077",
            "name": "白银"
          }
        ]
      },
      {
        "id": "881078",
        "code": "881078",
        "name": "能源金属",
        "children": [
          {
            "id": "881079",
            "code": "881079",
            "name": "锂"
          },
          {
            "id": "881080",
            "code": "881080",
            "name": "镍"
          },
          {
            "id": "881081",
            "code": "881081",
            "name": "钴"
          }
        ]
      },
      {
        "id": "881082",
        "code": "881082",
        "name": "稀有金属",
        "children": [
          {
            "id": "881083",
            "code": "881083",
            "name": "稀土"
          },
          {
            "id": "881084",
            "code": "881084",
            "name": "钨钼"
          },
          {
            "id": "881085",
            "code": "881085",
            "name": "钛"
          },
          {
            "id": "881086",
            "code": "881086",
            "name": "其他金属"
          }
        ]
      },
      {
        "id": "881087",
        "code": "881087",
        "name": "金属新材料",
        "children": [
          {
            "id": "881088",
            "code": "881088",
            "name": "磁性材料"
          },
          {
            "id": "881089",
            "code": "881089",
            "name": "其他金属新材料"
          }
        ]
      }
    ]
  },
  {
    "id": "881090",
    "code": "881090",
    "name": "建材",
    "children": [
      {
        "id": "881091",
        "code": "881091",
        "name": "水泥",
        "children": [
          {
            "id": "881092",
            "code": "881092",
            "name": "水泥制造"
          },
          {
            "id": "881093",
            "code": "881093",
            "name": "水泥制品"
          }
        ]
      },
      {
        "id": "881094",
        "code": "881094",
        "name": "玻璃玻纤",
        "children": [
          {
            "id": "881095",
            "code": "881095",
            "name": "玻璃制造"
          },
          {
            "id": "881096",
            "code": "881096",
            "name": "玻纤制造"
          }
        ]
      },
      {
        "id": "881097",
        "code": "881097",
        "name": "装饰建材",
        "children": [
          {
            "id": "881098",
            "code": "881098",
            "name": "陶瓷"
          },
          {
            "id": "881099",
            "code": "881099",
            "name": "管材"
          },
          {
            "id": "881100",
            "code": "881100",
            "name": "耐火材料"
          },
          {
            "id": "881101",
            "code": "881101",
            "name": "防水材料"
          },
          {
            "id": "881102",
            "code": "881102",
            "name": "涂料"
          },
          {
            "id": "881103",
            "code": "881103",
            "name": "其他建材"
          }
        ]
      }
    ]
  },
  {
    "id": "881105",
    "code": "881105",
    "name": "农林牧渔",
    "children": [
      {
        "id": "881106",
        "code": "881106",
        "name": "种植业",
        "children": [
          {
            "id": "881107",
            "code": "881107",
            "name": "粮食种植"
          },
          {
            "id": "881108",
            "code": "881108",
            "name": "食用菌"
          },
          {
            "id": "881109",
            "code": "881109",
            "name": "种子"
          },
          {
            "id": "881110",
            "code": "881110",
            "name": "其他种植业"
          }
        ]
      },
      {
        "id": "881111",
        "code": "881111",
        "name": "养殖业",
        "children": [
          {
            "id": "881112",
            "code": "881112",
            "name": "生猪养殖"
          },
          {
            "id": "881113",
            "code": "881113",
            "name": "肉鸡养殖"
          },
          {
            "id": "881114",
            "code": "881114",
            "name": "其他养殖"
          }
        ]
      },
      {
        "id": "881115",
        "code": "881115",
        "name": "林业",
        "children": []
      },
      {
        "id": "881116",
        "code": "881116",
        "name": "渔业",
        "children": [
          {
            "id": "881117",
            "code": "881117",
            "name": "水产养殖"
          },
          {
            "id": "881118",
            "code": "881118",
            "name": "水产捕捞"
          }
        ]
      },
      {
        "id": "881119",
        "code": "881119",
        "name": "饲料",
        "children": [
          {
            "id": "881120",
            "code": "881120",
            "name": "畜禽饲料"
          },
          {
            "id": "881121",
            "code": "881121",
            "name": "水产饲料"
          },
          {
            "id": "881122",
            "code": "881122",
            "name": "宠物食品"
          }
        ]
      },
      {
        "id": "881123",
        "code": "881123",
        "name": "农产品加工",
        "children": [
          {
            "id": "881124",
            "code": "881124",
            "name": "粮油加工"
          },
          {
            "id": "881125",
            "code": "881125",
            "name": "果蔬加工"
          },
          {
            "id": "881126",
            "code": "881126",
            "name": "其他农产品加工"
          }
        ]
      },
      {
        "id": "881127",
        "code": "881127",
        "name": "动物保健",
        "children": []
      }
    ]
  },
  {
    "id": "881129",
    "code": "881129",
    "name": "食品饮料",
    "children": [
      {
        "id": "881130",
        "code": "881130",
        "name": "酿酒",
        "children": [
          {
            "id": "881135",
            "code": "881135",
            "name": "白酒"
          },
          {
            "id": "881131",
            "code": "881131",
            "name": "啤酒"
          },
          {
            "id": "881132",
            "code": "881132",
            "name": "葡萄酒"
          },
          {
            "id": "881133",
            "code": "881133",
            "name": "黄酒"
          },
          {
            "id": "881134",
            "code": "881134",
            "name": "其他酒类"
          }
        ]
      },
      {
        "id": "881136",
        "code": "881136",
        "name": "饮料乳品",
        "children": [
          {
            "id": "881137",
            "code": "881137",
            "name": "软饮料"
          },
          {
            "id": "881138",
            "code": "881138",
            "name": "乳制品"
          }
        ]
      },
      {
        "id": "881139",
        "code": "881139",
        "name": "调味品",
        "children": []
      },
      {
        "id": "881140",
        "code": "881140",
        "name": "休闲食品",
        "children": [
          {
            "id": "881141",
            "code": "881141",
            "name": "零食"
          },
          {
            "id": "881142",
            "code": "881142",
            "name": "烘焙食品"
          },
          {
            "id": "881143",
            "code": "881143",
            "name": "熟食"
          }
        ]
      },
      {
        "id": "881144",
        "code": "881144",
        "name": "食品加工",
        "children": [
          {
            "id": "881145",
            "code": "881145",
            "name": "肉制品"
          },
          {
            "id": "881146",
            "code": "881146",
            "name": "制糖业"
          },
          {
            "id": "881147",
            "code": "881147",
            "name": "预加工食品"
          },
          {
            "id": "881148",
            "code": "881148",
            "name": "保健品"
          },
          {
            "id": "881149",
            "code": "881149",
            "name": "其他食品"
          }
        ]
      }
    ]
  },
  {
    "id": "881150",
    "code": "881150",
    "name": "纺织服饰",
    "children": [
      {
        "id": "881151",
        "code": "881151",
        "name": "纺织制造",
        "children": [
          {
            "id": "881152",
            "code": "881152",
            "name": "棉纺制品"
          },
          {
            "id": "881153",
            "code": "881153",
            "name": "印染"
          },
          {
            "id": "881154",
            "code": "881154",
            "name": "辅料"
          },
          {
            "id": "881155",
            "code": "881155",
            "name": "其他纺织"
          },
          {
            "id": "881156",
            "code": "881156",
            "name": "纺织鞋类制造"
          }
        ]
      },
      {
        "id": "881157",
        "code": "881157",
        "name": "服装家纺",
        "children": [
          {
            "id": "881158",
            "code": "881158",
            "name": "家纺"
          },
          {
            "id": "881159",
            "code": "881159",
            "name": "鞋帽及其他"
          },
          {
            "id": "881160",
            "code": "881160",
            "name": "运动服装"
          },
          {
            "id": "881161",
            "code": "881161",
            "name": "非运动服装"
          }
        ]
      },
      {
        "id": "881162",
        "code": "881162",
        "name": "饰品",
        "children": [
          {
            "id": "881163",
            "code": "881163",
            "name": "钟表珠宝"
          },
          {
            "id": "881164",
            "code": "881164",
            "name": "多品类奢侈品"
          },
          {
            "id": "881165",
            "code": "881165",
            "name": "其他饰品"
          }
        ]
      }
    ]
  },
  {
    "id": "881166",
    "code": "881166",
    "name": "轻工制造",
    "children": [
      {
        "id": "881167",
        "code": "881167",
        "name": "造纸",
        "children": [
          {
            "id": "881168",
            "code": "881168",
            "name": "大宗用纸"
          },
          {
            "id": "881169",
            "code": "881169",
            "name": "生活用纸"
          },
          {
            "id": "881170",
            "code": "881170",
            "name": "特种纸"
          }
        ]
      },
      {
        "id": "881171",
        "code": "881171",
        "name": "包装印刷",
        "children": [
          {
            "id": "881172",
            "code": "881172",
            "name": "印刷"
          },
          {
            "id": "881173",
            "code": "881173",
            "name": "金属包装"
          },
          {
            "id": "881174",
            "code": "881174",
            "name": "塑料包装"
          },
          {
            "id": "881175",
            "code": "881175",
            "name": "纸包装"
          },
          {
            "id": "881176",
            "code": "881176",
            "name": "其他包装"
          }
        ]
      },
      {
        "id": "881177",
        "code": "881177",
        "name": "家居用品",
        "children": [
          {
            "id": "881178",
            "code": "881178",
            "name": "家具"
          },
          {
            "id": "881179",
            "code": "881179",
            "name": "其他家居"
          }
        ]
      },
      {
        "id": "881180",
        "code": "881180",
        "name": "文娱用品",
        "children": [
          {
            "id": "881181",
            "code": "881181",
            "name": "文化用品"
          },
          {
            "id": "881182",
            "code": "881182",
            "name": "娱乐用品"
          }
        ]
      }
    ]
  },
  {
    "id": "881183",
    "code": "881183",
    "name": "家电",
    "children": [
      {
        "id": "881184",
        "code": "881184",
        "name": "白色家电",
        "children": [
          {
            "id": "881185",
            "code": "881185",
            "name": "空调"
          },
          {
            "id": "881186",
            "code": "881186",
            "name": "冰洗"
          }
        ]
      },
      {
        "id": "881187",
        "code": "881187",
        "name": "黑色家电",
        "children": [
          {
            "id": "881188",
            "code": "881188",
            "name": "电视"
          },
          {
            "id": "881189",
            "code": "881189",
            "name": "其他黑色家电"
          }
        ]
      },
      {
        "id": "881190",
        "code": "881190",
        "name": "小家电",
        "children": [
          {
            "id": "881191",
            "code": "881191",
            "name": "厨房小家电"
          },
          {
            "id": "881192",
            "code": "881192",
            "name": "清洁小家电"
          },
          {
            "id": "881193",
            "code": "881193",
            "name": "个护小家电"
          }
        ]
      },
      {
        "id": "881194",
        "code": "881194",
        "name": "厨卫电器",
        "children": [
          {
            "id": "881195",
            "code": "881195",
            "name": "厨房电器"
          },
          {
            "id": "881196",
            "code": "881196",
            "name": "卫浴电器"
          }
        ]
      },
      {
        "id": "881198",
        "code": "881198",
        "name": "家电零部件",
        "children": []
      }
    ]
  },
  {
    "id": "881199",
    "code": "881199",
    "name": "商贸",
    "children": [
      {
        "id": "881200",
        "code": "881200",
        "name": "一般零售",
        "children": [
          {
            "id": "881201",
            "code": "881201",
            "name": "超市及便利店"
          },
          {
            "id": "881202",
            "code": "881202",
            "name": "百货"
          },
          {
            "id": "881203",
            "code": "881203",
            "name": "多业态零售"
          }
        ]
      },
      {
        "id": "881204",
        "code": "881204",
        "name": "商业物业经营",
        "children": []
      },
      {
        "id": "881205",
        "code": "881205",
        "name": "专业连锁",
        "children": []
      },
      {
        "id": "881206",
        "code": "881206",
        "name": "贸易",
        "children": []
      },
      {
        "id": "881207",
        "code": "881207",
        "name": "电子商务",
        "children": [
          {
            "id": "881208",
            "code": "881208",
            "name": "综合电商"
          },
          {
            "id": "881209",
            "code": "881209",
            "name": "跨境电商"
          },
          {
            "id": "881210",
            "code": "881210",
            "name": "电商服务"
          }
        ]
      }
    ]
  },
  {
    "id": "881211",
    "code": "881211",
    "name": "汽车",
    "children": [
      {
        "id": "881212",
        "code": "881212",
        "name": "乘用车",
        "children": [
          {
            "id": "881213",
            "code": "881213",
            "name": "电动乘用车"
          },
          {
            "id": "881214",
            "code": "881214",
            "name": "综合乘用车"
          }
        ]
      },
      {
        "id": "881215",
        "code": "881215",
        "name": "商用车",
        "children": [
          {
            "id": "881216",
            "code": "881216",
            "name": "商用载货车"
          },
          {
            "id": "881217",
            "code": "881217",
            "name": "商用载客车"
          }
        ]
      },
      {
        "id": "881218",
        "code": "881218",
        "name": "汽车零部件",
        "children": [
          {
            "id": "881219",
            "code": "881219",
            "name": "车身附件及饰件"
          },
          {
            "id": "881220",
            "code": "881220",
            "name": "底盘与发动机系统"
          },
          {
            "id": "881221",
            "code": "881221",
            "name": "轮胎轮毂"
          },
          {
            "id": "881222",
            "code": "881222",
            "name": "其他汽车零部件"
          },
          {
            "id": "881223",
            "code": "881223",
            "name": "汽车电子电气系统"
          }
        ]
      },
      {
        "id": "881224",
        "code": "881224",
        "name": "汽车服务",
        "children": [
          {
            "id": "881225",
            "code": "881225",
            "name": "汽车经销商"
          },
          {
            "id": "881226",
            "code": "881226",
            "name": "汽车综合服务"
          }
        ]
      },
      {
        "id": "881227",
        "code": "881227",
        "name": "摩托车及其他",
        "children": [
          {
            "id": "881228",
            "code": "881228",
            "name": "摩托车"
          },
          {
            "id": "881229",
            "code": "881229",
            "name": "其他交通设备"
          }
        ]
      }
    ]
  },
  {
    "id": "881230",
    "code": "881230",
    "name": "医药医疗",
    "children": [
      {
        "id": "881231",
        "code": "881231",
        "name": "化学制药",
        "children": [
          {
            "id": "881232",
            "code": "881232",
            "name": "化学制剂"
          },
          {
            "id": "881233",
            "code": "881233",
            "name": "原料药"
          }
        ]
      },
      {
        "id": "881234",
        "code": "881234",
        "name": "生物制品",
        "children": [
          {
            "id": "881235",
            "code": "881235",
            "name": "血液制品"
          },
          {
            "id": "881236",
            "code": "881236",
            "name": "疫苗"
          },
          {
            "id": "881237",
            "code": "881237",
            "name": "抗体"
          },
          {
            "id": "881238",
            "code": "881238",
            "name": "诊断试剂"
          },
          {
            "id": "881239",
            "code": "881239",
            "name": "基因药物"
          },
          {
            "id": "881240",
            "code": "881240",
            "name": "其他生物制药"
          }
        ]
      },
      {
        "id": "881256",
        "code": "881256",
        "name": "中药",
        "children": []
      },
      {
        "id": "881252",
        "code": "881252",
        "name": "医药商业",
        "children": [
          {
            "id": "881253",
            "code": "881253",
            "name": "医药流通"
          },
          {
            "id": "881254",
            "code": "881254",
            "name": "线下药店"
          },
          {
            "id": "881255",
            "code": "881255",
            "name": "互联网药店"
          }
        ]
      },
      {
        "id": "881241",
        "code": "881241",
        "name": "医疗器械",
        "children": [
          {
            "id": "881242",
            "code": "881242",
            "name": "家用器械"
          },
          {
            "id": "881243",
            "code": "881243",
            "name": "医疗设备"
          },
          {
            "id": "881244",
            "code": "881244",
            "name": "医疗耗材"
          },
          {
            "id": "881245",
            "code": "881245",
            "name": "体外诊断"
          },
          {
            "id": "881246",
            "code": "881246",
            "name": "制药装备"
          }
        ]
      },
      {
        "id": "881247",
        "code": "881247",
        "name": "医疗服务",
        "children": [
          {
            "id": "881248",
            "code": "881248",
            "name": "诊断服务"
          },
          {
            "id": "881249",
            "code": "881249",
            "name": "医疗研发外包"
          },
          {
            "id": "881250",
            "code": "881250",
            "name": "医院"
          },
          {
            "id": "881251",
            "code": "881251",
            "name": "其他医疗服务"
          }
        ]
      },
      {
        "id": "881257",
        "code": "881257",
        "name": "医疗美容",
        "children": [
          {
            "id": "881258",
            "code": "881258",
            "name": "医美耗材"
          },
          {
            "id": "881259",
            "code": "881259",
            "name": "医美服务"
          }
        ]
      }
    ]
  },
  {
    "id": "881260",
    "code": "881260",
    "name": "电力设备",
    "children": [
      {
        "id": "881261",
        "code": "881261",
        "name": "电机制造",
        "children": []
      },
      {
        "id": "881262",
        "code": "881262",
        "name": "电池",
        "children": [
          {
            "id": "881263",
            "code": "881263",
            "name": "锂电池"
          },
          {
            "id": "881264",
            "code": "881264",
            "name": "电池化学品"
          },
          {
            "id": "881265",
            "code": "881265",
            "name": "锂电设备"
          },
          {
            "id": "881266",
            "code": "881266",
            "name": "燃料电池"
          },
          {
            "id": "881267",
            "code": "881267",
            "name": "其他电池"
          }
        ]
      },
      {
        "id": "881268",
        "code": "881268",
        "name": "电网设备",
        "children": [
          {
            "id": "881269",
            "code": "881269",
            "name": "电工仪器"
          },
          {
            "id": "881270",
            "code": "881270",
            "name": "电网自动化"
          },
          {
            "id": "881271",
            "code": "881271",
            "name": "输变电设备"
          },
          {
            "id": "881272",
            "code": "881272",
            "name": "配电设备"
          },
          {
            "id": "881273",
            "code": "881273",
            "name": "线缆部件及其他"
          }
        ]
      },
      {
        "id": "881275",
        "code": "881275",
        "name": "光伏设备",
        "children": [
          {
            "id": "881276",
            "code": "881276",
            "name": "光伏硅料"
          },
          {
            "id": "881277",
            "code": "881277",
            "name": "光伏硅片"
          },
          {
            "id": "881278",
            "code": "881278",
            "name": "光伏电池组件"
          },
          {
            "id": "881279",
            "code": "881279",
            "name": "光伏逆变器"
          },
          {
            "id": "881280",
            "code": "881280",
            "name": "光伏辅材"
          },
          {
            "id": "881281",
            "code": "881281",
            "name": "光伏加工设备"
          }
        ]
      },
      {
        "id": "881282",
        "code": "881282",
        "name": "风电设备",
        "children": [
          {
            "id": "881283",
            "code": "881283",
            "name": "风电整机"
          },
          {
            "id": "881284",
            "code": "881284",
            "name": "风电零部件"
          }
        ]
      },
      {
        "id": "881285",
        "code": "881285",
        "name": "其他发电设备",
        "children": []
      }
    ]
  },
  {
    "id": "881286",
    "code": "881286",
    "name": "国防军工",
    "children": [
      {
        "id": "881287",
        "code": "881287",
        "name": "地面兵装",
        "children": []
      },
      {
        "id": "881288",
        "code": "881288",
        "name": "航空装备",
        "children": []
      },
      {
        "id": "881289",
        "code": "881289",
        "name": "航天装备",
        "children": []
      },
      {
        "id": "881290",
        "code": "881290",
        "name": "航海装备",
        "children": []
      },
      {
        "id": "881291",
        "code": "881291",
        "name": "军工电子",
        "children": []
      }
    ]
  },
  {
    "id": "881292",
    "code": "881292",
    "name": "机械设备",
    "children": [
      {
        "id": "881293",
        "code": "881293",
        "name": "轨交设备",
        "children": []
      },
      {
        "id": "881294",
        "code": "881294",
        "name": "通用设备",
        "children": [
          {
            "id": "881295",
            "code": "881295",
            "name": "仪器仪表"
          },
          {
            "id": "881296",
            "code": "881296",
            "name": "机床制造"
          },
          {
            "id": "881297",
            "code": "881297",
            "name": "机械基础件"
          },
          {
            "id": "881298",
            "code": "881298",
            "name": "金属制品"
          },
          {
            "id": "881299",
            "code": "881299",
            "name": "内燃机"
          },
          {
            "id": "881300",
            "code": "881300",
            "name": "制冷空调设备"
          },
          {
            "id": "881301",
            "code": "881301",
            "name": "磨具磨料"
          },
          {
            "id": "881302",
            "code": "881302",
            "name": "其他通用设备"
          }
        ]
      },
      {
        "id": "881303",
        "code": "881303",
        "name": "专用设备",
        "children": [
          {
            "id": "881304",
            "code": "881304",
            "name": "能源及重型设备"
          },
          {
            "id": "881305",
            "code": "881305",
            "name": "印刷包装机械"
          },
          {
            "id": "881306",
            "code": "881306",
            "name": "纺织服装设备"
          },
          {
            "id": "881307",
            "code": "881307",
            "name": "农用机械"
          },
          {
            "id": "881308",
            "code": "881308",
            "name": "楼宇设备"
          },
          {
            "id": "881309",
            "code": "881309",
            "name": "其他专用设备"
          }
        ]
      },
      {
        "id": "881310",
        "code": "881310",
        "name": "工程机械",
        "children": [
          {
            "id": "881311",
            "code": "881311",
            "name": "工程机械整机"
          },
          {
            "id": "881312",
            "code": "881312",
            "name": "工程机械零件"
          }
        ]
      },
      {
        "id": "881313",
        "code": "881313",
        "name": "自动化设备",
        "children": [
          {
            "id": "881314",
            "code": "881314",
            "name": "机器人"
          },
          {
            "id": "881315",
            "code": "881315",
            "name": "工业控制设备"
          },
          {
            "id": "881316",
            "code": "881316",
            "name": "激光设备"
          },
          {
            "id": "881317",
            "code": "881317",
            "name": "其他自动化设备"
          }
        ]
      }
    ]
  },
  {
    "id": "881318",
    "code": "881318",
    "name": "电子",
    "children": [
      {
        "id": "881319",
        "code": "881319",
        "name": "半导体",
        "children": [
          {
            "id": "881320",
            "code": "881320",
            "name": "半导体材料"
          },
          {
            "id": "881321",
            "code": "881321",
            "name": "半导体设备"
          },
          {
            "id": "881322",
            "code": "881322",
            "name": "集成电路设计"
          },
          {
            "id": "881323",
            "code": "881323",
            "name": "功率半导体"
          },
          {
            "id": "881324",
            "code": "881324",
            "name": "半导体制造"
          },
          {
            "id": "881325",
            "code": "881325",
            "name": "半导体封测"
          }
        ]
      },
      {
        "id": "881326",
        "code": "881326",
        "name": "消费电子",
        "children": [
          {
            "id": "881327",
            "code": "881327",
            "name": "消费电子组件"
          },
          {
            "id": "881328",
            "code": "881328",
            "name": "品牌消费电子"
          }
        ]
      },
      {
        "id": "881329",
        "code": "881329",
        "name": "光学光电",
        "children": [
          {
            "id": "881330",
            "code": "881330",
            "name": "面板"
          },
          {
            "id": "881331",
            "code": "881331",
            "name": "光学元件"
          },
          {
            "id": "881332",
            "code": "881332",
            "name": "LED"
          }
        ]
      },
      {
        "id": "881333",
        "code": "881333",
        "name": "元器件",
        "children": [
          {
            "id": "881334",
            "code": "881334",
            "name": "PCB"
          },
          {
            "id": "881335",
            "code": "881335",
            "name": "被动元件"
          }
        ]
      },
      {
        "id": "881336",
        "code": "881336",
        "name": "其他电子",
        "children": []
      }
    ]
  },
  {
    "id": "881337",
    "code": "881337",
    "name": "通信",
    "children": [
      {
        "id": "881338",
        "code": "881338",
        "name": "通信设备",
        "children": [
          {
            "id": "881339",
            "code": "881339",
            "name": "光纤光缆"
          },
          {
            "id": "881340",
            "code": "881340",
            "name": "网络接配及塔设"
          },
          {
            "id": "881341",
            "code": "881341",
            "name": "系统设备"
          },
          {
            "id": "881342",
            "code": "881342",
            "name": "通信终端及配件"
          },
          {
            "id": "881343",
            "code": "881343",
            "name": "其他通信设备"
          }
        ]
      },
      {
        "id": "881347",
        "code": "881347",
        "name": "通信工程",
        "children": [
          {
            "id": "881348",
            "code": "881348",
            "name": "网络规划设计"
          },
          {
            "id": "881349",
            "code": "881349",
            "name": "网络工程施工"
          },
          {
            "id": "881350",
            "code": "881350",
            "name": "网络优化运维"
          }
        ]
      },
      {
        "id": "881344",
        "code": "881344",
        "name": "电信服务",
        "children": [
          {
            "id": "881345",
            "code": "881345",
            "name": "电信运营服务"
          },
          {
            "id": "881346",
            "code": "881346",
            "name": "电信增值服务"
          }
        ]
      }
    ]
  },
  {
    "id": "881351",
    "code": "881351",
    "name": "计算机",
    "children": [
      {
        "id": "881352",
        "code": "881352",
        "name": "IT设备",
        "children": [
          {
            "id": "881353",
            "code": "881353",
            "name": "安防设备"
          },
          {
            "id": "881354",
            "code": "881354",
            "name": "其他IT设备"
          }
        ]
      },
      {
        "id": "881355",
        "code": "881355",
        "name": "软件服务",
        "children": [
          {
            "id": "881356",
            "code": "881356",
            "name": "基础软件"
          },
          {
            "id": "881357",
            "code": "881357",
            "name": "行业应用软件"
          },
          {
            "id": "881358",
            "code": "881358",
            "name": "新兴软件"
          }
        ]
      },
      {
        "id": "881359",
        "code": "881359",
        "name": "云服务",
        "children": [
          {
            "id": "881360",
            "code": "881360",
            "name": "云基础设施服务"
          },
          {
            "id": "881361",
            "code": "881361",
            "name": "云平台服务"
          },
          {
            "id": "881362",
            "code": "881362",
            "name": "云软件服务"
          },
          {
            "id": "881363",
            "code": "881363",
            "name": "其他云服务"
          }
        ]
      },
      {
        "id": "881364",
        "code": "881364",
        "name": "产业互联网",
        "children": [
          {
            "id": "881365",
            "code": "881365",
            "name": "产业互联网信息服务"
          },
          {
            "id": "881366",
            "code": "881366",
            "name": "产业互联网平台服务"
          },
          {
            "id": "881367",
            "code": "881367",
            "name": "产业互联网综合服务"
          }
        ]
      }
    ]
  },
  {
    "id": "881368",
    "code": "881368",
    "name": "传媒",
    "children": [
      {
        "id": "881369",
        "code": "881369",
        "name": "游戏",
        "children": []
      },
      {
        "id": "881370",
        "code": "881370",
        "name": "广告营销",
        "children": [
          {
            "id": "881371",
            "code": "881371",
            "name": "互联网广告"
          },
          {
            "id": "881372",
            "code": "881372",
            "name": "其他广告营销"
          }
        ]
      },
      {
        "id": "881373",
        "code": "881373",
        "name": "影视院线",
        "children": [
          {
            "id": "881374",
            "code": "881374",
            "name": "影视制作"
          },
          {
            "id": "881375",
            "code": "881375",
            "name": "发行院线"
          }
        ]
      },
      {
        "id": "881376",
        "code": "881376",
        "name": "数字媒体",
        "children": [
          {
            "id": "881377",
            "code": "881377",
            "name": "视频音频媒体"
          },
          {
            "id": "881378",
            "code": "881378",
            "name": "文字图片媒体"
          },
          {
            "id": "881379",
            "code": "881379",
            "name": "其他数字媒体"
          }
        ]
      },
      {
        "id": "881380",
        "code": "881380",
        "name": "出版业",
        "children": [
          {
            "id": "881381",
            "code": "881381",
            "name": "教育出版"
          },
          {
            "id": "881382",
            "code": "881382",
            "name": "大众出版"
          },
          {
            "id": "881383",
            "code": "881383",
            "name": "其他出版"
          }
        ]
      },
      {
        "id": "881384",
        "code": "881384",
        "name": "广播电视",
        "children": []
      }
    ]
  },
  {
    "id": "881385",
    "code": "881385",
    "name": "银行",
    "children": [
      {
        "id": "881386",
        "code": "881386",
        "name": "全国性银行",
        "children": [
          {
            "id": "881387",
            "code": "881387",
            "name": "国有大型银行"
          },
          {
            "id": "881388",
            "code": "881388",
            "name": "股份制银行"
          }
        ]
      },
      {
        "id": "881389",
        "code": "881389",
        "name": "地方性银行",
        "children": [
          {
            "id": "881390",
            "code": "881390",
            "name": "城商行"
          },
          {
            "id": "881391",
            "code": "881391",
            "name": "农商行"
          }
        ]
      }
    ]
  },
  {
    "id": "881393",
    "code": "881393",
    "name": "非银金融",
    "children": [
      {
        "id": "881394",
        "code": "881394",
        "name": "证券",
        "children": []
      },
      {
        "id": "881395",
        "code": "881395",
        "name": "保险",
        "children": []
      },
      {
        "id": "881396",
        "code": "881396",
        "name": "多元金融",
        "children": [
          {
            "id": "881397",
            "code": "881397",
            "name": "非货币银行"
          },
          {
            "id": "881398",
            "code": "881398",
            "name": "信托"
          },
          {
            "id": "881399",
            "code": "881399",
            "name": "投资管理"
          },
          {
            "id": "881400",
            "code": "881400",
            "name": "期货"
          },
          {
            "id": "881401",
            "code": "881401",
            "name": "租赁"
          },
          {
            "id": "881402",
            "code": "881402",
            "name": "金融控股"
          },
          {
            "id": "881403",
            "code": "881403",
            "name": "金融信息服务"
          },
          {
            "id": "881404",
            "code": "881404",
            "name": "其他金融服务"
          }
        ]
      }
    ]
  },
  {
    "id": "881405",
    "code": "881405",
    "name": "建筑",
    "children": [
      {
        "id": "881406",
        "code": "881406",
        "name": "房屋建设",
        "children": []
      },
      {
        "id": "881407",
        "code": "881407",
        "name": "基础建设",
        "children": [
          {
            "id": "881408",
            "code": "881408",
            "name": "基建工程"
          },
          {
            "id": "881409",
            "code": "881409",
            "name": "园林工程"
          }
        ]
      },
      {
        "id": "881410",
        "code": "881410",
        "name": "专业工程",
        "children": [
          {
            "id": "881411",
            "code": "881411",
            "name": "化学工程"
          },
          {
            "id": "881412",
            "code": "881412",
            "name": "钢结构"
          },
          {
            "id": "881413",
            "code": "881413",
            "name": "国际工程"
          },
          {
            "id": "881414",
            "code": "881414",
            "name": "其他专业工程"
          }
        ]
      },
      {
        "id": "881415",
        "code": "881415",
        "name": "工程咨询服务",
        "children": []
      },
      {
        "id": "881416",
        "code": "881416",
        "name": "装修装饰",
        "children": []
      }
    ]
  },
  {
    "id": "881417",
    "code": "881417",
    "name": "房地产",
    "children": [
      {
        "id": "881418",
        "code": "881418",
        "name": "房地产开发",
        "children": [
          {
            "id": "881419",
            "code": "881419",
            "name": "住宅开发"
          },
          {
            "id": "881420",
            "code": "881420",
            "name": "商业地产"
          },
          {
            "id": "881421",
            "code": "881421",
            "name": "产业地产"
          }
        ]
      },
      {
        "id": "881422",
        "code": "881422",
        "name": "房产服务",
        "children": [
          {
            "id": "881423",
            "code": "881423",
            "name": "物业管理"
          },
          {
            "id": "881424",
            "code": "881424",
            "name": "租赁经纪"
          }
        ]
      }
    ]
  },
  {
    "id": "881426",
    "code": "881426",
    "name": "社会服务",
    "children": [
      {
        "id": "881427",
        "code": "881427",
        "name": "体育",
        "children": []
      },
      {
        "id": "881428",
        "code": "881428",
        "name": "教育培训",
        "children": []
      },
      {
        "id": "881429",
        "code": "881429",
        "name": "酒店餐饮",
        "children": [
          {
            "id": "881430",
            "code": "881430",
            "name": "酒店"
          },
          {
            "id": "881431",
            "code": "881431",
            "name": "餐饮"
          }
        ]
      },
      {
        "id": "881432",
        "code": "881432",
        "name": "旅游",
        "children": [
          {
            "id": "881433",
            "code": "881433",
            "name": "旅游景区"
          },
          {
            "id": "881434",
            "code": "881434",
            "name": "旅游服务"
          },
          {
            "id": "881435",
            "code": "881435",
            "name": "休闲综合"
          }
        ]
      },
      {
        "id": "881436",
        "code": "881436",
        "name": "专业服务",
        "children": [
          {
            "id": "881437",
            "code": "881437",
            "name": "人力资源服务"
          },
          {
            "id": "881438",
            "code": "881438",
            "name": "检测服务"
          },
          {
            "id": "881439",
            "code": "881439",
            "name": "会展服务"
          },
          {
            "id": "881440",
            "code": "881440",
            "name": "其他专业服务"
          }
        ]
      }
    ]
  },
  {
    "id": "881441",
    "code": "881441",
    "name": "交通运输",
    "children": [
      {
        "id": "881442",
        "code": "881442",
        "name": "公路铁路",
        "children": [
          {
            "id": "881445",
            "code": "881445",
            "name": "公共交通"
          },
          {
            "id": "881444",
            "code": "881444",
            "name": "高速公路"
          },
          {
            "id": "881443",
            "code": "881443",
            "name": "铁路运输"
          }
        ]
      },
      {
        "id": "881446",
        "code": "881446",
        "name": "航空机场",
        "children": [
          {
            "id": "881447",
            "code": "881447",
            "name": "航空运输"
          },
          {
            "id": "881448",
            "code": "881448",
            "name": "机场"
          }
        ]
      },
      {
        "id": "881449",
        "code": "881449",
        "name": "航运港口",
        "children": [
          {
            "id": "881450",
            "code": "881450",
            "name": "港口"
          },
          {
            "id": "881451",
            "code": "881451",
            "name": "航运"
          }
        ]
      },
      {
        "id": "881452",
        "code": "881452",
        "name": "物流",
        "children": [
          {
            "id": "881453",
            "code": "881453",
            "name": "供应链服务"
          },
          {
            "id": "881454",
            "code": "881454",
            "name": "快递"
          },
          {
            "id": "881455",
            "code": "881455",
            "name": "跨境物流"
          },
          {
            "id": "881456",
            "code": "881456",
            "name": "仓储物流"
          },
          {
            "id": "881457",
            "code": "881457",
            "name": "公路货运"
          }
        ]
      }
    ]
  },
  {
    "id": "881458",
    "code": "881458",
    "name": "公用事业",
    "children": [
      {
        "id": "881459",
        "code": "881459",
        "name": "电力",
        "children": [
          {
            "id": "881460",
            "code": "881460",
            "name": "水力发电"
          },
          {
            "id": "881461",
            "code": "881461",
            "name": "火力发电"
          },
          {
            "id": "881462",
            "code": "881462",
            "name": "热力服务"
          },
          {
            "id": "881463",
            "code": "881463",
            "name": "风力发电"
          },
          {
            "id": "881464",
            "code": "881464",
            "name": "光伏发电"
          },
          {
            "id": "881465",
            "code": "881465",
            "name": "核力发电"
          },
          {
            "id": "881466",
            "code": "881466",
            "name": "电能综合服务"
          }
        ]
      },
      {
        "id": "881467",
        "code": "881467",
        "name": "燃气",
        "children": []
      },
      {
        "id": "881468",
        "code": "881468",
        "name": "水务",
        "children": []
      }
    ]
  },
  {
    "id": "881469",
    "code": "881469",
    "name": "环保",
    "children": [
      {
        "id": "881470",
        "code": "881470",
        "name": "环保设备",
        "children": []
      },
      {
        "id": "881471",
        "code": "881471",
        "name": "环境治理",
        "children": [
          {
            "id": "881472",
            "code": "881472",
            "name": "大气治理"
          },
          {
            "id": "881473",
            "code": "881473",
            "name": "水治理"
          },
          {
            "id": "881474",
            "code": "881474",
            "name": "固废治理"
          },
          {
            "id": "881475",
            "code": "881475",
            "name": "生态修复"
          }
        ]
      },
      {
        "id": "881476",
        "code": "881476",
        "name": "环境监测",
        "children": []
      }
    ]
  },
  {
    "id": "881477",
    "code": "881477",
    "name": "综合",
    "children": []
  }
];

const STORAGE_KEY = "industryWatchlist_v3";
const statusList = ["观察中", "已选股", "已放弃"];

let watchRecords = loadRecords();
let toastTimer = null;
let pickerState = {
  level: 1,
  selectedLevel1: null,
  selectedLevel2: null
};

const elements = {
  industryTree: document.querySelector("#industryTree"),
  watchlist: document.querySelector("#watchlist"),
  watchCount: document.querySelector("#watchCount"),
  searchInput: document.querySelector("#searchInput"),
  level1Filter: document.querySelector("#level1Filter"),
  statusFilter: document.querySelector("#statusFilter"),
  starFilter: document.querySelector("#starFilter"),
  toast: document.querySelector("#toast")
};

function loadRecords() {
  try {
    const saved = localStorage.getItem(STORAGE_KEY);
    const records = saved ? JSON.parse(saved) : [];
    return Array.isArray(records) ? records : [];
  } catch (error) {
    console.warn("观察池数据读取失败，已使用空数据。", error);
    return [];
  }
}

function saveRecords() {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(watchRecords));
}

function formatDate(date = new Date()) {
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const day = String(date.getDate()).padStart(2, "0");
  return `${year}-${month}-${day}`;
}

function formatIndustryLabel(industry) {
  return `[${industry.code}] ${industry.name}`;
}

function formatRecordLabel(code, name) {
  return `[${code}] ${name}`;
}

function hasChildren(industry) {
  return Array.isArray(industry.children) && industry.children.length > 0;
}

function isAdded(selectedCode) {
  return watchRecords.some((record) => record.selectedCode === selectedCode);
}

function renderLevel1Filters() {
  elements.level1Filter.innerHTML = '<option value="all">全部</option>';
  industryData.forEach((level1) => {
    const option = document.createElement("option");
    option.value = level1.id;
    option.textContent = formatIndustryLabel(level1);
    elements.level1Filter.appendChild(option);
  });
}

function renderIndustryPicker() {
  elements.industryTree.innerHTML = "";
  elements.industryTree.appendChild(createPickerHeader());

  if (pickerState.level === 1) {
    renderLevel1Picker();
    return;
  }

  if (pickerState.level === 2) {
    renderLevel2Picker();
    return;
  }

  renderLevel3Picker();
}

function createPickerHeader() {
  const header = document.createElement("div");
  header.className = "picker-meta";

  const path = document.createElement("div");
  path.className = "picker-path";

  const label = document.createElement("span");
  label.className = "picker-path-label";
  label.textContent = "当前选择";

  const value = document.createElement("span");
  value.textContent = getCurrentPickerPath();

  path.append(label, value);
  header.appendChild(path);

  const actions = document.createElement("div");
  actions.className = "picker-actions";

  if (pickerState.level === 2) {
    actions.appendChild(createPickerBackButton("返回一级行业", resetPickerToLevel1));
  }

  if (pickerState.level === 3) {
    actions.append(
      createPickerBackButton("返回二级行业", backToLevel2),
      createPickerBackButton("返回一级行业", resetPickerToLevel1)
    );
  }

  if (actions.children.length > 0) {
    header.appendChild(actions);
  }

  return header;
}

function getCurrentPickerPath() {
  if (pickerState.level === 1 || !pickerState.selectedLevel1) {
    return "一级行业";
  }

  const level1Label = formatIndustryLabel(pickerState.selectedLevel1);
  if (pickerState.level === 2 || !pickerState.selectedLevel2) {
    return level1Label;
  }

  return `${level1Label} / ${formatIndustryLabel(pickerState.selectedLevel2)}`;
}

function createPickerBackButton(text, onClick) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "picker-back-btn";
  button.textContent = text;
  button.addEventListener("click", onClick);
  return button;
}

function renderLevel1Picker() {
  const list = createPickerList();

  industryData.forEach((level1) => {
    if (hasChildren(level1)) {
      const item = createPickerNavItem(level1, "选择一级行业");
      item.addEventListener("click", () => {
        pickerState.level = 2;
        pickerState.selectedLevel1 = level1;
        pickerState.selectedLevel2 = null;
        renderIndustryPicker();
      });
      list.appendChild(item);
      return;
    }

    list.appendChild(createPickerLeafItem(level1, () => addToWatchlist(level1)));
  });

  elements.industryTree.appendChild(list);
}

function renderLevel2Picker() {
  const level1 = pickerState.selectedLevel1;
  if (!level1) {
    resetPickerToLevel1();
    return;
  }

  if (!hasChildren(level1)) {
    elements.industryTree.appendChild(createPickerEmptyState("该一级行业暂无二级行业。"));
    return;
  }

  const list = createPickerList();
  level1.children.forEach((level2) => {
    if (hasChildren(level2)) {
      const item = createPickerNavItem(level2, "选择二级行业");
      item.addEventListener("click", () => {
        pickerState.level = 3;
        pickerState.selectedLevel2 = level2;
        renderIndustryPicker();
      });
      list.appendChild(item);
      return;
    }

    list.appendChild(createPickerLeafItem(level2, () => addToWatchlist(level1, level2)));
  });

  elements.industryTree.appendChild(list);
}

function renderLevel3Picker() {
  const level1 = pickerState.selectedLevel1;
  const level2 = pickerState.selectedLevel2;
  if (!level1) {
    resetPickerToLevel1();
    return;
  }

  if (!level2) {
    backToLevel2();
    return;
  }

  if (!hasChildren(level2)) {
    elements.industryTree.appendChild(createPickerEmptyState("该二级行业暂无三级行业。"));
    return;
  }

  const list = createPickerList();
  level2.children.forEach((level3) => {
    list.appendChild(createPickerLeafItem(level3, () => addToWatchlist(level1, level2, level3)));
  });

  elements.industryTree.appendChild(list);
}

function createPickerList() {
  const list = document.createElement("div");
  list.className = "picker-list";
  return list;
}

function createPickerNavItem(industry, title) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "picker-item picker-nav-item";
  button.title = title;

  const name = document.createElement("span");
  name.className = "picker-item-title";
  name.textContent = formatIndustryLabel(industry);

  const arrow = document.createElement("span");
  arrow.className = "picker-arrow";
  arrow.textContent = "›";
  arrow.setAttribute("aria-hidden", "true");

  button.append(name, arrow);
  return button;
}

function createPickerLeafItem(industry, onAdd) {
  const row = document.createElement("div");
  row.className = "picker-item picker-leaf-item";

  const name = document.createElement("span");
  name.className = "picker-item-title";
  name.textContent = formatIndustryLabel(industry);

  const alreadyAdded = isAdded(industry.code);
  const actions = document.createElement("div");
  actions.className = "picker-item-actions";

  const addButton = document.createElement("button");
  addButton.type = "button";
  addButton.className = "add-btn";
  addButton.textContent = alreadyAdded ? "已加入" : "+";
  addButton.disabled = alreadyAdded;
  addButton.setAttribute("aria-label", `加入 ${formatIndustryLabel(industry)} 到观察池`);
  addButton.addEventListener("click", onAdd);
  actions.appendChild(addButton);

  if (alreadyAdded) {
    const copyButton = document.createElement("button");
    copyButton.type = "button";
    copyButton.className = "copy-code-btn";
    copyButton.textContent = "复制代码";
    copyButton.setAttribute("aria-label", `复制 ${formatIndustryLabel(industry)} 的行业代码`);
    copyButton.addEventListener("click", (event) => {
      event.stopPropagation();
      copyIndustryCode(industry.code);
    });
    actions.appendChild(copyButton);
  }

  row.append(name, actions);
  return row;
}

function createPickerEmptyState(message) {
  const empty = document.createElement("div");
  empty.className = "empty-state picker-empty";
  empty.textContent = message;
  return empty;
}

function resetPickerToLevel1() {
  pickerState.level = 1;
  pickerState.selectedLevel1 = null;
  pickerState.selectedLevel2 = null;
  renderIndustryPicker();
}

function backToLevel2() {
  pickerState.level = 2;
  pickerState.selectedLevel2 = null;
  renderIndustryPicker();
}

function addToWatchlist(level1, level2 = null, level3 = null) {
  const selectedIndustry = level3 || level2 || level1;
  const selectedLevel = level3 ? 3 : level2 ? 2 : 1;

  if (isAdded(selectedIndustry.code)) {
    showToast("该行业已在观察池中");
    renderIndustryPicker();
    return;
  }

  const today = formatDate();
  watchRecords.unshift({
    id: selectedIndustry.code,
    selectedLevel,
    selectedId: selectedIndustry.id,
    selectedCode: selectedIndustry.code,
    selectedName: selectedIndustry.name,
    level1Id: level1.id,
    level1Code: level1.code,
    level1Name: level1.name,
    level2Id: level2 ? level2.id : null,
    level2Code: level2 ? level2.code : null,
    level2Name: level2 ? level2.name : null,
    level3Id: level3 ? level3.id : null,
    level3Code: level3 ? level3.code : null,
    level3Name: level3 ? level3.name : null,
    status: "观察中",
    isStarred: false,
    note: "",
    createdAt: today,
    updatedAt: today
  });

  saveRecords();
  renderAll();
  showToast("已加入观察池");
}

async function copyIndustryCode(code) {
  const codeText = String(code);
  let clipboardError = null;

  if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
    try {
      await navigator.clipboard.writeText(codeText);
      showToast(`已复制代码：${codeText}`);
      return;
    } catch (error) {
      clipboardError = error;
    }
  }

  try {
    fallbackCopyText(codeText);
    showToast(`已复制代码：${codeText}`);
  } catch (error) {
    console.warn("行业代码复制失败。", clipboardError || error);
    showToast("复制失败，请手动复制");
  }
}

function fallbackCopyText(text) {
  const textarea = document.createElement("textarea");
  textarea.value = text;
  textarea.setAttribute("readonly", "");
  textarea.style.position = "fixed";
  textarea.style.left = "-9999px";
  textarea.style.top = "0";
  document.body.appendChild(textarea);
  textarea.select();
  textarea.setSelectionRange(0, textarea.value.length);

  try {
    const copied = document.execCommand("copy");
    if (!copied) {
      throw new Error("document.execCommand copy returned false");
    }
  } finally {
    document.body.removeChild(textarea);
  }
}

function getFilteredRecords() {
  const keyword = elements.searchInput.value.trim().toLowerCase();
  const level1Value = elements.level1Filter.value;
  const statusValue = elements.statusFilter.value;
  const starValue = elements.starFilter.value;

  return watchRecords.filter((record) => {
    const searchText = [
      record.selectedName,
      record.selectedCode,
      record.level1Id,
      record.level1Code,
      record.level1Name,
      record.level2Id,
      record.level2Code,
      record.level2Name,
      record.level3Id,
      record.level3Code,
      record.level3Name,
      record.note
    ].filter(Boolean).join(" ").toLowerCase();

    const matchesKeyword = !keyword || searchText.includes(keyword);
    const matchesLevel1 = level1Value === "all" || record.level1Id === level1Value;
    const matchesStatus = statusValue === "all" || record.status === statusValue;
    const matchesStar = starValue === "all" || record.isStarred;

    return matchesKeyword && matchesLevel1 && matchesStatus && matchesStar;
  });
}

function renderWatchlist() {
  const filteredRecords = getFilteredRecords();
  elements.watchCount.textContent = `${watchRecords.length} 条`;
  elements.watchlist.innerHTML = "";

  if (watchRecords.length === 0) {
    elements.watchlist.innerHTML = '<div class="empty-state">还没有加入任何行业。请从左侧行业选择器中添加无下级行业。</div>';
    return;
  }

  if (filteredRecords.length === 0) {
    elements.watchlist.innerHTML = '<div class="empty-state">没有符合当前筛选条件的记录。</div>';
    return;
  }

  filteredRecords.forEach((record) => {
    elements.watchlist.appendChild(createWatchCard(record));
  });
}

function createWatchCard(record) {
  const card = document.createElement("article");
  card.className = "watch-card";

  const top = document.createElement("div");
  top.className = "card-top";

  const titleBox = document.createElement("div");

  const titleLine = document.createElement("div");
  titleLine.className = "title-line";

  const industryName = document.createElement("span");
  industryName.className = "industry-name";
  industryName.textContent = formatRecordLabel(record.selectedCode, record.selectedName);
  titleLine.appendChild(industryName);

  const path = document.createElement("div");
  path.className = "path";
  path.textContent = getRecordPath(record);

  titleBox.append(titleLine, path);

  const actions = document.createElement("div");
  actions.className = "card-actions";

  const starButton = document.createElement("button");
  starButton.type = "button";
  starButton.className = `star-btn${record.isStarred ? " is-starred" : ""}`;
  starButton.textContent = record.isStarred ? "★" : "☆";
  starButton.title = record.isStarred ? "取消星标" : "标记星标";
  starButton.addEventListener("click", () => toggleStar(record.id));

  const statusButton = document.createElement("button");
  statusButton.type = "button";
  statusButton.className = `status-btn ${getStatusClass(record.status)}`;
  statusButton.textContent = record.status;
  statusButton.title = "点击切换状态";
  statusButton.addEventListener("click", () => toggleStatus(record.id));

  const deleteButton = document.createElement("button");
  deleteButton.type = "button";
  deleteButton.className = "delete-btn";
  deleteButton.textContent = "删";
  deleteButton.title = "删除记录";
  deleteButton.addEventListener("click", () => deleteRecord(record.id));

  actions.append(starButton, statusButton, deleteButton);
  top.append(titleBox, actions);

  const noteLabel = document.createElement("label");
  noteLabel.className = "note-label";
  noteLabel.textContent = "备注";

  const noteInput = document.createElement("textarea");
  noteInput.placeholder = "点击备注";
  noteInput.value = record.note;
  noteInput.addEventListener("input", (event) => updateNote(record.id, event.target.value));
  noteLabel.appendChild(noteInput);

  const meta = document.createElement("div");
  meta.className = "meta";
  meta.innerHTML = `<span>创建时间：${record.createdAt}</span><span>更新时间：${record.updatedAt}</span>`;

  card.append(top, noteLabel, meta);
  return card;
}


function getRecordPath(record) {
  const labels = [
    formatRecordLabel(record.level1Code, record.level1Name),
    record.level2Code && record.level2Name ? formatRecordLabel(record.level2Code, record.level2Name) : null,
    record.level3Code && record.level3Name ? formatRecordLabel(record.level3Code, record.level3Name) : null
  ].filter(Boolean);

  return labels.length > 1 ? labels.join(" / ") : "一级行业";
}

function getStatusClass(status) {
  if (status === "已选股") return "status-done";
  if (status === "已放弃") return "status-drop";
  return "";
}

function findRecord(recordId) {
  return watchRecords.find((record) => record.id === recordId);
}

function touchRecord(record) {
  record.updatedAt = formatDate();
}

function toggleStatus(recordId) {
  const record = findRecord(recordId);
  if (!record) return;

  const currentIndex = statusList.indexOf(record.status);
  record.status = statusList[(currentIndex + 1) % statusList.length];
  touchRecord(record);
  saveRecords();
  renderWatchlist();
}

function toggleStar(recordId) {
  const record = findRecord(recordId);
  if (!record) return;

  record.isStarred = !record.isStarred;
  touchRecord(record);
  saveRecords();
  renderWatchlist();
}

function updateNote(recordId, note) {
  const record = findRecord(recordId);
  if (!record) return;

  record.note = note;
  touchRecord(record);
  saveRecords();
}

function deleteRecord(recordId) {
  const record = findRecord(recordId);
  if (!record) return;

  const confirmed = window.confirm(`确定删除「${formatRecordLabel(record.selectedCode, record.selectedName)}」吗？`);
  if (!confirmed) return;

  watchRecords = watchRecords.filter((item) => item.id !== recordId);
  saveRecords();
  renderAll();
  showToast("已删除观察记录");
}

function showToast(message) {
  window.clearTimeout(toastTimer);
  elements.toast.textContent = message;
  elements.toast.classList.add("show");
  toastTimer = window.setTimeout(() => {
    elements.toast.classList.remove("show");
  }, 1800);
}

function renderAll() {
  renderIndustryPicker();
  renderWatchlist();
}

function bindFilters() {
  [
    elements.searchInput,
    elements.level1Filter,
    elements.statusFilter,
    elements.starFilter
  ].forEach((element) => {
    element.addEventListener("input", renderWatchlist);
    element.addEventListener("change", renderWatchlist);
  });
}

renderLevel1Filters();
bindFilters();
renderAll();
